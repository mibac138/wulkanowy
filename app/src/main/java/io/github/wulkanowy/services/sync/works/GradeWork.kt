        ...
        for ((student, semester) in students) {
            ...
            gradeRepository.getGradesFromDatabase(semester).first()
                .filter { !it.isNotified }.let {
                    if (it.isNotEmpty()) newGradeNotification.notifyDetails(student.nickOrName,
                        it,
                        student)
            ...
            gradeRepository.getGradesPredictedFromDatabase(semester).first()
                .filter { !it.isPredictedGradeNotified }.let {
                    if (it.isNotEmpty()) newGradeNotification.notifyPredicted(student.nickOrName,
                        it,
                        student)
            ...
            gradeRepository.getGradesFinalFromDatabase(semester).first()
                .filter { !it.isFinalGradeNotified }.let {
                    if (it.isNotEmpty()) newGradeNotification.notifyFinal(student.nickOrName,
                        it,
                        student)
