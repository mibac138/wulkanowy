...
class DashboardAdapter @Inject constructor() : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private fun bindLowAttendanceSubjectsHolder(holder: LowAttendanceSubjectsViewHolder, position: Int) {
        val item = items[position] as DashboardItem.LowAttendanceSubjects
        val list = item.summariesWithNames.orEmpty()
        val error = item.error
        val isLoading = item.isLoading
        val context = holder.binding.root.context
        val adapter = holder.adapter.apply {
            this.items = list.filter { it.second.calculatePercentage() <= ATTENDANCE_FIRST_WARNING_THRESHOLD }.take(MAX_VISIBLE_LIST_ITEMS)
        }
